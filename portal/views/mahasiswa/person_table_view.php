<table>
	<tr>
		<td valign="top" rowspan="<?php echo count($data)-2; ?>"><img src="<?php echo site_url()."assets/uploads/".$data['photo_image']; ?>" width="200" /></td>
		<td><?php echo $this->lang->line('name') ?></td><td><?php echo $data['name']; ?></td>
	</tr>	
	<?php foreach($data as $key=>$value){ if(!in_array($key,array('name','uid','password','photo_image','teach','teach_at','teach_major','last_education_major'))){?>
	<tr>
		<?php if(!in_array($key,array('passport_image','ijasah_image'))) { 
			...
			<?php }elseif($key=='region'){ ?>
				<td><?php echo $this->lang->line($key); ?></td><td><?php echo (in_array($value,$region_arr)?$region_arr[$value]:$value); ?></td>
