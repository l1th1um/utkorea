...
"SELECT")&&!d)&&e===true){H=w[0].id;T=a.jgrid.getCellIndex(z);ca=a(z).closest("td,th").html();a(c).triggerHandler("jqGridCellSelect",[H,T,ca,b]);a.isFunction(c.p.onCellSelect)&&c.p.onCellSelect.call(c,H,T,ca,b);if(c.p.cellEdit===true)if(c.p.multiselect&&d)a(c).jqGrid("setSelection",H,true,b);else{H=w[0].rowIndex;try{a(c).jqGrid("editCell",H,T,true)}catch(f){}}else if(c.p.multikey)if(b[c.p.multikey])a(c).jqGrid("setSelection",H,true,b);else{if(c.p.multiselect&&d){d=a("#jqg_"+a.jgrid.jqID(c.p.id)+"_"+
H).is(":checked");a("#jqg_"+a.jgrid.jqID(c.p.id)+"_"+H)[c.p.useProp?"prop":"attr"]("checked",d)}}else{if(c.p.multiselect&&c.p.multiboxonly&&!d){var g=c.p.frozenColumns?c.p.id+"_frozen":"";a(c.p.selarrrow).each(function(b,d){var e=c.rows.namedItem(d);a(e).removeClass("ui-state-highlight");a("#jqg_"+a.jgrid.jqID(c.p.id)+"_"+a.jgrid.jqID(d))[c.p.useProp?"prop":"attr"]("checked",false);if(g){a("#"+a.jgrid.jqID(d),"#"+a.jgrid.jqID(g)).removeClass("ui-state-highlight");a("#jqg_"+a.jgrid.jqID(c.p.id)+"_"+
a.jgrid.jqID(d),"#"+a.jgrid.jqID(g))[c.p.useProp?"prop":"attr"]("checked",false)}});c.p.selarrrow=[]}a(c).jqGrid("setSelection",H,true,b)}}}).bind("reloadGrid",function(b,d){if(c.p.treeGrid===true)c.p.datatype=c.p.treedatatype;d&&d.current&&c.grid.selectionPreserver(c);if(c.p.datatype=="local"){a(c).jqGrid("resetSelection");c.p.data.length&&x()}else if(!c.p.treeGrid){c.p.selrow=null;if(c.p.multiselect){c.p.selarrrow=[];J(false)}c.p.savedRow=[]}c.p.scroll&&y.call(c,true,false);if(d&&d.page){var e=
